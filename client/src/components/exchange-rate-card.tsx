      ...
      <Card className="bg-white rounded-lg shadow-md p-6 mb-6 border border-red-200">
        <CardContent className="p-0">
          <div className="flex items-center text-red-500">
            ...
            <span>Error loading exchange rate data. Please try again.</span>
          </div>
        </CardContent>
      </Card>
    );
        ...
        <Card className="bg-white rounded-lg shadow-md p-6 mb-6 border border-gray-100 transition-all hover:shadow-lg">
          <CardContent className="p-0">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
              <div>
                <h2 className="text-lg font-semibold text-gray-800">Current Exchange Rate</h2>
                <p className="text-sm text-gray-500">Last updated: {lastUpdated}</p>
              </div>
              
              {isSuggestionLoading ? (
                <div className="mt-4 md:mt-0 bg-gray-100 py-2 px-4 rounded-full animate-pulse">
                  <span className="text-gray-500 text-sm">Loading suggestion...</span>
                </div>
              ) : (
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    <p className="font-mono font-medium text-xl">
                      {isRateLoading ? "Loading..." : vndAmount.toLocaleString()}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-6 flex justify-between items-center">
              <div>
                ...
                <p className="font-mono font-semibold text-lg">
                  {isRateLoading ? "Loading..." : `${rateData?.rate.toFixed(2)} VND per JPY`}
                </p>
              </div>
